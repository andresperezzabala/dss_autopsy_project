...
//        System.out.println(confMatrix.);
    ...
    /**
     * Genera un HashMap de instancias cuya clave serÃ¡ el conjunto de atributos que se representan mediante un String
    ...
    public static HashSet<String> generateHashSet(Instances pData, String[] pKeys) {        	
    	...
    	for(Instance i: pData) { //Recorrer todas las instancias    		
    		data.add(generatekey(pData, pKeys, i)); // AÃ±adir al HashSet la clave    	 	
    	}   
    	return data;
    }
    ...
    public static String generatekey(Instances pData, String[] pKeys, Instance i) {
    	...
    	for (String k : pKeys) { //Genera la clave por cada atributo requerido
			key += Double.toString(i.value(pData.attribute(k))); //AÃ±adir a la clave el valor del atributo k en la instancia i
		}
    	return key;
    }
