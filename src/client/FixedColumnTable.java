...
/*
 *  by Rob Camick on November 5, 2008
...
public class FixedColumnTable implements ChangeListener, PropertyChangeListener
{
	...
	public void updateFixedTable(double[] values) {
		
		for (int i = 0; i < values.length; i++) {
			fixed.setValueAt(values[i], i, 1);
		}
...
//  Implement the PropertyChangeListener
	...
	//  Keep the fixed table in sync with the main table
		if ("selectionModel".equals(arg0.getPropertyName()))
		{
			fixed.setSelectionModel( main.getSelectionModel() );
		}

		if ("model".equals(arg0.getPropertyName()))
		{
			fixed.setModel( main.getModel() );
		}		
	}
